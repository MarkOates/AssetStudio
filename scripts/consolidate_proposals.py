...
def consolidate(args):
    if len(args) % 2 != 0:
        print("Usage: python consolidate_proposals.py <rel_path1> <json_file1> [<rel_path2> <json_file2> ...]")
        sys.exit(1)
        
    proposals_path = os.path.join(os.path.dirname(__file__), 'ai_subagent_proposals.json')
    new_proposals = []
    ...
    for i in range(0, len(args), 2):
        ...
        vendor = parts[0]
        pack = parts[1] if len(parts) > 1 and parts[1] != "extracted" else vendor
        ...
        audit_block["pass1_timestamp"] = __import__('datetime').datetime.now(__import__('datetime').timezone.utc).isoformat()
    ...
    if not new_proposals:
        print("No proposals to consolidate.")
        return
    ...
    print(f"✅ Successfully consolidated {len(new_proposals)} proposals into {proposals_path}")
