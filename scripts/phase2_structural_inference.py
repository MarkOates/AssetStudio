    ...
    for idx, directory in enumerate(dir_list):
        ...
        try:
            ...
            for pa in result_data.get("parent_assets", []):
                audit_block = pa.get("ai_audit", {})
                audit_block["pass2_model"] = "gemini-1.5-pro"
                audit_block["pass2_timestamp"] = datetime.datetime.now(datetime.timezone.utc).isoformat()
                pa["ai_audit"] = audit_block
