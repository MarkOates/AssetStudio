FLAGS_PATH = "web/flags.json"
VIEWER_DATA_PATH = "web/viewer_data.json"
AI_PROPOSALS = "ai_catalog_proposals.json"
SUBAGENT_PROPOSALS = "ai_subagent_proposals.json"
import os
import json
import hashlib
import time
...
def process_asset(rel_path):
    ...
    try:
        ...
        vendor = parts[0]
        pack = parts[1] if "extracted" not in parts[1] else parts[2]
        ...
        asset_dict = {
            ...
            "ai_audit": {
                "pass1_model": "gemini-3.5-flash-lite-concurrent",
...
def main():
    ...
    error_ids = [k for k, v in flags.items() if v == 'error']
    print(f"Found {len(error_ids)} assets marked as error.")
    if not error_ids:
        return
        
    print("Loading viewer_data.json...")
    with open(VIEWER_DATA_PATH, 'r', encoding='utf-8') as f:
        vdata = json.load(f)
    
    # Map identifiers to their relative path
    id_to_path = {}
    for a in vdata.get('assets', []):
        if a['identifier'] in error_ids:
            # Get the first source file and strip /Assets/
            sf = a['resource']['source_files'][0]
            rel_path = sf.replace('/Assets/', '', 1)
            id_to_path[a['identifier']] = rel_path
            
    print(f"Mapped {len(id_to_path)} source files for reprocessing.")
    
    # Process concurrently
    results = []
    print(f"Reprocessing {len(id_to_path)} assets with 10 workers...")
    import time
    start_time = time.time()
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        futures = {executor.submit(process_asset, path): path for path in id_to_path.values()}
        completed = 0
        for future in concurrent.futures.as_completed(futures):
            ...
            if res:
                results.append(res)
                print(f"[{completed}/{len(id_to_path)}] Processed {res['name']}")
                
    duration = time.time() - start_time
    print(f"Finished processing in {duration:.1f}s.")
    
    # Now, append the new ones to ai_catalog_proposals.json
    # AND optionally remove the old ones, but since we are generating new ones with the exact same identifier,
    # we should filter the existing ai_catalog_proposals.json and ai_subagent_proposals.json
    
    # Clear errors from flags.json
    for k in error_ids:
        del flags[k]
    with open(FLAGS_PATH, 'w', encoding='utf-8') as f:
        json.dump(flags, f)
        
    for p_path in ['scripts/' + AI_PROPOSALS, 'scripts/' + SUBAGENT_PROPOSALS]:
        if not os.path.exists(p_path): continue
        with open(p_path, 'r', encoding='utf-8') as f:
            old_proposals = json.load(f)
        filtered = [p for p in old_proposals if p['identifier'] not in error_ids]
        with open(p_path, 'w', encoding='utf-8') as f:
            json.dump(filtered, f, indent=2)
            
    # Add new results to AI_PROPOSALS
    ap_path = 'scripts/' + AI_PROPOSALS
    ...
    print("Done! Reprocessed files injected into AI proposals.")
