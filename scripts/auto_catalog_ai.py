...
def main():
    ...
    for f in uncat_files:
        if f not in proposed_paths:
            physical_path = os.path.join(ASSETS_DIR, f)
            if os.path.exists(physical_path):
                if os.path.getsize(physical_path) > 10 * 1024 * 1024:
                    print(f"Skipping very large file (>10MB): {f}")
                    continue
                valid_uncat.append(f)
    uncat_files = valid_uncat
    
    # Process all remaining files
    batch = uncat_files[:20000]
...
DO NOT wrap the response in markdown blocks like ```json. Just return the raw JSON object."""
    ...
    for idx, rel_path in enumerate(batch):
        ...
        try:
            ...
            temp_path = PROPOSALS_PATH + '.tmp'
            with open(temp_path, 'w', encoding='utf-8') as f:
                json.dump(proposals, f, indent=2)
            os.rename(temp_path, PROPOSALS_PATH)
